...
int main( int argc, char* argv[] )
{
    ...
    #ifdef CALIPER
    cali_id_t attr_phase = cali_create_attribute("phase", CALI_TYPE_STRING, CALI_ATTR_SKIP_EVENTS|CALI_ATTR_DEFAULT);
    cali_begin(attr_phase, "main", strlen("main"));
    cali_begin(attr_phase, "generate_nuclide_grids", strlen("generate_nuclide_grids"));
    #endif
    ...
    #ifdef CALIPER
    cali_end(attr_phase);
    cali_begin(attr_phase, "sort_nuclide_grids", strlen("sort_nuclide_grids"));
    #endif
    ...
    #ifdef CALIPER
    cali_end(attr_phase);
    cali_begin(attr_phase, "allocate_energy_grid", strlen("allocate_energy_grid"));
    #endif
    ...
    #ifdef CALIPER
    cali_end(attr_phase);
    cali_begin(attr_phase, "load_materials", strlen("load_materials"));
    #endif
    ...
    #ifdef CALIPER
    cali_end(attr_phase);
    cali_begin(attr_phase, "xs_simulation", strlen("xs_simulation"));
    cali_id_t attr_lookup = cali_create_attribute("lookup", CALI_TYPE_INT, CALI_ATTR_SKIP_EVENTS|CALI_ATTR_ASVALUE);
    #endif
...
#pragma omp parallel default(none) \
	...
	shared( max_procs, in, energy_grid, nuclide_grids, \
            ...
            attr_phase, attr_lookup) // CALIPER
	{	
		...
		for( i = 0; i < in.lookups; i++ )
		{
            #ifdef CALIPER
            if(i%1000 == 0)
                cali_set(attr_lookup, &i, sizeof(int64_t));
            #endif
