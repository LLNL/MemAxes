...
{
#ifdef MITOS
    MITOS_ALLOC(int, num_nucs, 12, 1, 1);
#else
...
#ifdef MITOS
        char matname[16];
        sprintf(matname, "mats[%d]", i);
        size_t dims[] = {num_nucs[i]};
        Mitos_add_symbol(matname, mats[i], sizeof(int), dims, 1);
#endif
    }
...
#ifdef MITOS
        char concname[16];
        sprintf(concname, "concs[%d]", i);
        size_t dims[] = {num_nucs[i]};
        Mitos_add_symbol(concname, concs[i], sizeof(double), dims, 1);
#endif
    }
