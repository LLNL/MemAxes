...
struct mitos_output mout;

void sample_handler(struct perf_event_sample *sample, void *args)
{
...
#ifdef MITOS
    char rank_prefix[16];
    sprintf(rank_prefix, "rank_%d", mype);
...
#pragma omp parallel
	...
	#else
#ifdef MITOS
    MITOS_ALLOC(GridPoint, energy_grid, in.n_isotopes, in.n_gridpoints);
    MITOS_ALLOC(int, index_data, in.n_isotopes, in.n_gridpoints, in.n_isotopes);
#else
